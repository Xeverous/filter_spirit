...
auto make_path_settings(el::host_window_handle window, view& v, user_state& state)
{
	...
	auto on_filter_template_browse = [label_filter_template_path, window, &v, &state](bool) mutable {
...
auto make_builtin_options_api_selection(view& v, user_state& state)
{
	// -- league selection --
	...
	state.generation.league_state.refresh_available_leagues = [=, &v, &state]() mutable {
		if (state.generation.league_state.download_running)
	...
	state.generation.league_state.update = [=, &v, &state]() {
	...
	state.generation.item_price_data_state.refresh_item_price_data = [=, &v, &state]() mutable {
	...
	item_price_data_refresh_button->on_click = [&state](bool) {
		state.generation.item_price_data_state.refresh_item_price_data();
	};

	state.generation.item_price_data_state.update = [=, &v, &state]() {
...
auto make_builtin_options(view& v, user_state& state)
{
	return
		el::group("built-in generation options", el::margin(group_margin_size(),
			el::vtile(
				...
				make_builtin_options_api_selection(v, state)
			)
		));
}
...
auto make_tab_main(el::host_window_handle window, view& v, user_state& state)
{
	return
		el::vtile(
			el::htile(
				el::vtile(
					make_path_settings(window, v, state),
					make_builtin_options(v, state),
