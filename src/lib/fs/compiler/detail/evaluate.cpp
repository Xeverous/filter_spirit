...
{
	const auto name_origin = position_tag_of(name);
...
{
	lang::socket_spec ss;
	ss.origin = position_tag_of(literal.socket_colors);
	...
	if (raw_letters.empty()) {
		 ...
		 */
		diagnostics.push_error_internal_compiler_error(__func__, position_tag_of(literal.socket_colors));
		return boost::none;
	}
	...
	for (std::size_t i = 0; i < raw_letters.size(); ++i) {
		...
		if (c == kw::r)
			...
			diagnostics.push_message(make_error(
				diagnostic_message_id::invalid_socket_spec,
				position_tag_of(literal.socket_colors),
	...
	if (!ss.is_valid()) {
		diagnostics.push_message(make_error(
			diagnostic_message_id::invalid_socket_spec, ss.origin, "invalid socket group"));
	...
	diagnostics_store& diagnostics)
{
	...
	return expression.apply_visitor(make_lambda_visitor<result_type>(
		[&](const auto& literal) {
...
evaluate_literal_sequence(
	settings st,
	const ast::rf::literal_sequence& sequence,
	diagnostics_store& diagnostics)
{
	...
	return lang::object{std::move(seq_values), position_tag_of(sequence)};
}
	...
	diagnostics_store& diagnostics)
{
	...
	for (const ast::sf::primitive_value& primitive : sequence) {
		...
		auto result = primitive.apply_visitor(make_lambda_visitor<result_type>(
			[&](const ast::sf::name& name) {
			...
			[&](const ast::common::literal_expression& expr) -> result_type {
				return evaluate(st, expr, diagnostics)
					.map([&](lang::single_object so) {
						return lang::object{{std::move(so)}, position_tag_of(expr)};
					}
				);
			},
...
check_object_size(
	...
	diagnostics_store& diagnostics)
{
	FS_ASSERT(min_allowed_elements <= max_allowed_elements.value_or(min_allowed_elements));
	const auto num_elements = static_cast<int>(obj.values.size());
		...
		|| (max_allowed_elements && num_elements > *max_allowed_elements))
	{
		diagnostics.push_error_invalid_amount_of_arguments(
			min_allowed_elements, max_allowed_elements, num_elements, obj.origin);
		return false;
	}
