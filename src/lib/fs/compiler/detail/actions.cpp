	...
	diagnostics_store& diagnostics)
{
	...
	if (none.has_value()) {
		...
		// main error
		diagnostics.push_message(make_error(dmid::invalid_play_effect, obj.origin, "invalid ", lang::keywords::rf::play_effect));
	...
	diagnostics_store& diagnostics)
{
	...
	if (none_sound_id.has_value()) {
		...
		// main error
		diagnostics.push_message(make_error(dmid::invalid_alert_sound_id, sobj.origin, "invalid value for alert sound ID"));
	...
	diagnostics_store& diagnostics)
{
	return detail::evaluate_sequence(st, action.seq, symbols, diagnostics)
		.flat_map([&](lang::object obj) -> boost::optional<lang::alert_sound> {
			...
			boost::optional<lang::volume> vol = lang::volume{};
			diagnostics_store diagnostics_volume;
			if (obj.values.size() == 2u) {
			...
			if (custom_alert.has_value() && vol.has_value()) {
				...
				diagnostics.move_messages_from(diagnostics_volume);
	...
	diagnostics_store& diagnostics)
{
	...
	diagnostics.push_error_internal_compiler_error(__func__, act.origin);
	return false;
}
