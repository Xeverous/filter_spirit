...
{
	if (st.is_ruthless() && visibility.value == lang::item_visibility_policy::hide) {
	...
	if (!st.is_ruthless() && visibility.value == lang::item_visibility_policy::minimal) {
		diagnostics.push_error_invalid_statement(
			position_tag_of(visibility),
	...
	diagnostics_store& diagnostics)
{
	...
	if (st.is_ruthless() && policy == lang::item_visibility_policy::hide) {
		diagnostics.push_error_invalid_statement(
			position_tag_of(visibility),
	...
	if (!st.is_ruthless() && policy == lang::item_visibility_policy::minimal) {
		diagnostics.push_error_invalid_statement(
			position_tag_of(visibility),
...
std::string make_preamble(lang::game_variant_type game_variant, const lang::market::item_price_metadata& metadata)
{
	std::string preamble =
...
"#     game variant              : " + std::string(to_string(game_variant)) + "\n"
...
compile_real_filter(
	...
	diagnostics_store& diagnostics)
{
	lang::item_filter filter(st.game_variant);
	...
	diagnostics_store& diagnostics)
{
	...
	return lang::spirit_item_filter{st.game_variant, std::move(*blocks)};
}
...
std::string item_filter_to_string_without_preamble(const lang::item_filter& filter, lang::style_overrides overrides)
{
	std::stringstream ss;
...
{
	return make_preamble(filter.game_variant, item_price_metadata) + item_filter_to_string_without_preamble(filter, overrides);
}
	...
	log::logger& logger)
{
	...
	return make_preamble(st.game_variant, report.metadata) + std::move(*maybe_filter);
}
